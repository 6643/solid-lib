...
export const AppTap = (props: {
    children: { icon: string, panel: () => any }[]
}) => {
    const key = "app.tab"
    ...
    return <div class={styles.AppTap}>
        <For each={props.children}>{({ panel }, index) =>
            <Show when={index() == getActiveIndex()}>
                <main ref={(el) => useKeepScroll(el, location.pathname, getTabScrollKey(index()))} class={isToLeft() ? styles.moveRight : styles.moveLeft}>{panel()}</main>
            </Show>
        }</For>

        <nav style={{ "--len": props.children.length, "--index": getActiveIndex(), "--deg": getDeg() }}>
            <For each={props.children}>{({ icon }, index) =>
