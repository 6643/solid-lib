...
export const LeftTab = (props: {
    ...
    return <div ref={(ele) => el = ele} class={styles.leftTab} part={mode} >
            ...
            <Match when={mode == "all"}>
                <For each={props.children}>{({ panel }, index) =>
                    <Show when={index() == getActiveIndex()}>
                        <main ref={(el) => useKeepScroll(el, location.pathname, getTabScrollKey(index()))} class={isToUp() ? styles.moveUp : styles.moveDown}>{panel()}</main>
                    </Show>
                }</For>
            </Match>

            <Match when={mode == "part"}>
                <main ref={(el) => useKeepScroll(el, location.pathname, getTabScrollKey())} onScroll={scrollEnd}>
                    <For each={props.children}>{({ panel }, index) =>
