...
import { SvgIcon, icon_expand_all, icon_unfold_less } from "./SvgIcon.tsx"
export const TimeLine = (props: {
    ...
    return <div class={styles.TimeLine}>
        <div>{props.title}</div>
        <For each={getEvents()}>{({ time, info, url }) =>
            <Show when={url} fallback={<Item time={time} info={info} />}>
                <a href={url}><Item time={time} info={info} /></a>
            </Show>
        }</For>
        <Show when={props.children.length > initEvents.length}>
            <main>
                ...
                <div onClick={toggle}>
                    ...
                    <SvgIcon color="var(--grey5-color)" name={getEvents().length != props.children.length ? icon_expand_all : icon_unfold_less} />
                </div>
            </main>
        </Show>
    </div>
