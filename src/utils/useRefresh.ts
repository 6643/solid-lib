...
export const useRefresh = (
    ...
    createTrackedEffect(() => {
        ...
        createTrackedEffect(() => {
            el.classList.toggle(styles.pulling!, getPullState() === PullState.Pulling)
            el.classList.toggle(styles.ready!, getPullState() === PullState.Ready)
            el.classList.toggle(styles.loading!, getPullState() === PullState.Loading)
            el.classList.toggle(styles.done!, getPullState() === PullState.Done)
        })

        createTrackedEffect(() => {
            el.style.setProperty("--pull-height", getCurrentIndicatorHeight().toString())
        })
