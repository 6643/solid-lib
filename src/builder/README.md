...
- `watchDirs` 必须是字符串数组, 且每个路径必须是已存在的目录 (允许相对路径指向项目外, 例如 monorepo 场景下的 `../src`)
- 入口组件必须有默认导出
...
默认监听 `0.0.0.0` 以便局域网访问. 在不受信任的网络上可通过环境变量收紧:

```bash
SOLID_LIB_DEV_HOST=127.0.0.1 solid-lib dev
```

### 生产构建原子替换

`solid-lib build` 先写入临时目录, 成功后再替换 `outDir`. 构建失败时不会清空既有产物.
