    ...
    <div class={styles.page}>
      ...
      <FlexBox gap={16} wrap="wrap">
        ...
        <Card class={styles.card}>
          ...
          <FlexBox gap={16} dir="column">
            ...
            <TextInput label="备注" row={3} changed={(v) => console.log("备注:", v)} validate={(v) => v.length < 5 ? "至少输入5个字符" : undefined} />
          </FlexBox>
        </Card>
      </FlexBox>
      ...
      <FlexBox gap={16} wrap="wrap">
        <Card class={styles.card}>
          ...
          <FlexBox gap={16} dir="column">
            <NumberInput label="年龄" value={age()} changed={setAge} min={0} max={150} step={1} />
      ...
      <Card class={styles.card}>
        ...
        <div class={styles.codeBlock}>
          <pre>{JSON.stringify({ name: name(), email: email(), phone: phone(), age: age(), bio: bio(), volume: volume() }, null, 2)}</pre>
        </div>
      </Card>
    </div>
  );
